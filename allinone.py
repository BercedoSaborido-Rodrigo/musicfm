...
class AllInOne(nn.Module):
  def __init__(self, cfg: Config):
    ...
    # self.embeddings = AllInOneEmbeddings(cfg)  ##rehacer estopor lo embedings q vienen d fuera?¿¿?¿?¿?
    
    self.musicfm_embeddings=musicfmembeddings()
    self.linear1 = nn.Linear(1024, 512)  
    self.linear2 = nn.Linear(512, cfg.dim_embed) 
...
#se supone q tengo q añadir aquí los nuevos cosos    
  ...
  def forward(
    ...
    #embedings q generamos
    musicfm_emb = self.musicfm_embeddings.gener_emb(audio_path)

    # cambio de dimensiones
    emb_transformed = self.linear1(musicfm_emb)
    emb_transformed = torch.relu(emb_transformed)
    emb_transformed = self.linear2(emb_transformed)
...
class musicfmembeddings(nn.Module):
    def __init__(self):
        super(musicfmembeddings, self).__init__()
    ...
    def formato_entrada_musicfm(self,waveform,sample_rate):
    ...
    def gener_emb(audoipath):
        waveform,sample_rate = self.carga_fma(audio_path)
        logits, hidden_emb, losses, accuracies = musicfm(waveform) #traga spectrogramas no wavs recordar (good Harmonix), falso¿?
        return hidden_emb[7]
