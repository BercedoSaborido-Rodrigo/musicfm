...
from all_in_one.src.allin1.models.allinone import musicfmembeddings
...
################################################################################
      # Cambiamos esto para adaptar los nuevos emb
    # with torch.no_grad():
    ...
    with torch.no_grad():
        ...
        for path, spec_path in pbar:
            ...
            # generamos embedings:
            waveform, sample_rate = musiffmembeddings.carga_Fma(path)  #
            musicfm_embeddings = musiffmembeddings.gener_emb(waveform)  
            ...
            result = run_inference(
              ...
              device=device,
              include_activations=include_activations,
              include_embeddings=True,  # comprobamos si los embedings son
            )
    
            # MusicFM embeddings into the result
            result['custom_musicfm_embeddings'] = musicfm_embeddings
